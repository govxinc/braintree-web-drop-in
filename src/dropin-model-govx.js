...
DropinModel.prototype._shouldEmitRequestableEvent = function (options) {
  var requestableStateHasNotChanged =
    this.isPaymentMethodRequestable() === options.isRequestable;
  var nonce =
    options.selectedPaymentMethod && options.selectedPaymentMethod.nonce;
  ...
  if (
    requestableStateHasNotChanged &&
    (!options.isRequestable || nonceHasNotChanged)
  ) {
    return false;
  }
...
DropinModel.prototype.hasAtLeastOneAvailablePaymentOption = function () {
  ...
  for (i = 0; i < this.supportedPaymentOptions.length; i++) {
    if (
      self.dependencyStates[this.supportedPaymentOptions[i]] ===
...
DropinModel.prototype.getVaultedPaymentMethods = function () {
  ...
  return self._vaultManager
    ...
    .then(function (paymentMethods) {
      return self
        ._getSupportedPaymentMethods(paymentMethods)
...
DropinModel.prototype._getSupportedPaymentOptions = function (options) {
  var self = this;
  var paymentOptionPriority =
    options.merchantConfiguration.paymentOptionPriority ||
...
function getPaymentOption(paymentOption, options) {
  return isPaymentOptionEnabled(paymentOption, options).then(function (
    success
  ) {
    return {
      success: success,
...
function canShowVaultedPaymentMethodType(
  ...
  return (
    paymentMethodType &&
    hiddenVaultedPaymentMethodTypes.indexOf(paymentMethodType) === -1
  );
}
